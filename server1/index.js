...
app.post("/send_email", (req, res) => {
    ...
    if (status == "Alert") {
        ...
        const subject = `Emergency!! your family member ${name} is in trouble`;
        const text = `Dear Family Member,
I am writing to inform you that your family member has been detected as drowsy while driving and is currently unresponsive. We have sent this email to you as an emergency measure to ensure you can take appropriate action.

The location of the vehicle has been tracked and is currently at the following location: ${encodedLocation}. 

We urge you to immediately come to the location or try contacting ${name} on their phone number ${phone} to ensure their safety.

Sincerely,
[Drowsiness detection team]`;

        sendTelegramMessage(
            `This is a message for ${email1} and ${email2}\n${text}`
        );
    ...
    if (flag == true) {
        console.log(`this is the logged text - ${text}`);
        sendTelegramMessage(
            `This is a message for ${email1} and ${email2}\n${text}`
        );
    }
